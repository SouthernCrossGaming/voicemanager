...
bool IsHearingClient(IClient* pToClient, int fromClientSlot)
{
	...
	if (!pWrapper)
	{
		...
		pWrapper = g_pBinTools->CreateCall(pIsHearingClientAddress, CallConv_ThisCall, &ret, pass, 1);
	}

// Fraeven: CGameClient for linux is offset by - 1. ¯\_(ツ)_/¯
#ifdef PLATFORM_LINUX
	IClient* pToGameClient = pToClient - 1;
#else
	IClient* pToGameClient = pToClient;
#endif

	ArgBuffer<IClient*, int> vstk(pToGameClient, fromClientSlot);
...
bool VoiceManagerExt::SDK_OnLoad(char* error, size_t maxlength, bool late)
{
	...
	if (!g_pGameConf->GetMemSig("IsHearingClient", &pIsHearingClientAddress))
	{
		smutils->LogError(myself, "IsHearingClient signature not defined");
		return false;
	}
